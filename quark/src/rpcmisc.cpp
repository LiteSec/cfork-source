...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total cfork.litesec.name.lower balance of the wallet\n"
            "  \"obfuscation_balance\": xxxxxx, (numeric) the anonymized cfork.litesec.name.lower balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in cfork.litesec.name.lower/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in cfork.litesec.name.lower/kb\n"
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"cfork.litesec.name.loweraddress\"\n"
            "\nReturn information about the given cfork.litesec.name.lower address.\n"
            "\nArguments:\n"
            "1. \"cfork.litesec.name.loweraddress\"     (string, required) The cfork.litesec.name.lower address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"cfork.litesec.name.loweraddress\", (string) The cfork.litesec.name.lower address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: cfork.litesec.name.cap address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are cfork.litesec.name.lower addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) cfork.litesec.name.lower address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"cfork.litesec.name.loweraddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"cfork.litesec.name.loweraddress\"  (string, required) The cfork.litesec.name.lower address to use for the signature.\n"
