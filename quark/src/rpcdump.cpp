...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey \"cfork.litesec.name.lowerprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"cfork.litesec.name.lowerprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
Value importaddress(const Array& params, bool fHelp)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid cfork.litesec.name.cap address or script");
    }
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey \"cfork.litesec.name.loweraddress\"\n"
            "\nReveals the private key corresponding to 'cfork.litesec.name.loweraddress'.\n"
            "Then the importprivkey can be used with this output\n"
            "\nArguments:\n"
            "1. \"cfork.litesec.name.loweraddress\"   (string, required) The cfork.litesec.name.lower address for the private key\n"
            "\nResult:\n"
...
Value dumpwallet(const Array& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by cfork.litesec.name.cap %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
...
Value bip38encrypt(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "bip38encrypt \"cfork.litesec.name.loweraddress\"\n"
            "\nEncrypts a private key corresponding to 'cfork.litesec.name.loweraddress'.\n"
            "\nArguments:\n"
            "1. \"cfork.litesec.name.loweraddress\"   (string, required) The cfork.litesec.name.lower address for the private key (you must hold the key already)\n"
...
Value bip38decrypt(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "bip38decrypt \"cfork.litesec.name.loweraddress\"\n"
