...
#include "config/cfork.litesec.name.lower-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("cfork.litesec.name.lower-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "cfork.litesec.name.lower.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "cfork.litesec.name.lowerd.pid"));
#endif
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in cfork.litesec.ticker/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in cfork.litesec.ticker/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, cfork.litesec.name.lower, (obfuscation, swifttx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in cfork.litesec.ticker/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all cfork.litesec.name.cap specific functionality (Masternodes, Obfuscation, SwiftTX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-anonymizecfork.litesec.name.loweramount=<n>", strprintf(_("Keep N cfork.litesec.ticker anonymized (default: %u)"), 0));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(_("Copyright (C) 2018 The cfork.litesec.name.cap Core Developers")) + "\n" +
           "\n" +
...
{
    RenameThread("cfork.litesec.name.lower-loadblk");
...
/** Sanity checks
 *  Ensure that cfork.litesec.name.cap is running in a usable environment with all
...
/** Initialize cfork.litesec.name.lower.
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. cfork.litesec.name.cap Core is shutting down."));
...
#endif
    // Make sure only a single cfork.litesec.name.cap process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. cfork.litesec.name.cap Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("cfork.litesec.name.cap version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of cfork.litesec.name.cap Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart cfork.litesec.name.cap Core to complete") << "\n";
    ...
    nAnonymizecfork.litesec.name.capAmount = GetArg("-anonymizecfork.litesec.name.loweramount", 0);
    if (nAnonymizecfork.litesec.name.capAmount > 999999) nAnonymizecfork.litesec.name.capAmount = 999999;
    if (nAnonymizecfork.litesec.name.capAmount < 2) nAnonymizecfork.litesec.name.capAmount = 2;
    ...
    LogPrintf("Anonymize cfork.litesec.name.cap Amount %d\n", nAnonymizecfork.litesec.name.capAmount);
