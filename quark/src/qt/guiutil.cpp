...
#define URI_SCHEME "cfork.litesec.name.lower"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a cfork.litesec.name.cap address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no cfork.litesec.name.cap: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++) {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::cfork.litesec.ticker, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert cfork.litesec.name.lower:// to cfork.litesec.name.lower:
    //
    //    Cannot handle this later, because cfork.litesec.name.lower:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::cfork.litesec.ticker, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open cfork.litesec.name.lower.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "cfork.litesec.name.cap.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for cfork.litesec.name.cap.lnk
...
{
    return GetAutostartDir() / "cfork.litesec.name.lower.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a cfork.litesec.name.lower.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=cfork.litesec.name.cap\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the cfork.litesec.name.lower app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add cfork.litesec.name.lower app to startup item list
