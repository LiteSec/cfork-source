...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::cfork.litesec.ticker)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("cfork.litesec.ticker")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizecfork.litesec.name.capAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizecfork.litesec.name.capAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizecfork.litesec.name.capAmount = strAnonymizecfork.litesec.name.capAmount.remove(strAnonymizecfork.litesec.name.capAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizecfork.litesec.name.capAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizecfork.litesec.name.capAmount * COIN) nMaxToAnonymize = nAnonymizecfork.litesec.name.capAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizecfork.litesec.name.capAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizecfork.litesec.name.capAmount));
        strAnonymizecfork.litesec.name.capAmount = strAnonymizecfork.litesec.name.capAmount.remove(strAnonymizecfork.litesec.name.capAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizecfork.litesec.name.capAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
...
void OverviewPage::toggleObfuscation()
{
    ...
    if (!fEnableObfuscation) {
        ...
        if (nAnonymizecfork.litesec.name.capAmount == 0) {
