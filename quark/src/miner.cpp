...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("cfork.litesec.name.capMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("cfork.litesec.name.capMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("cfork.litesec.name.capMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cfork.litesec.name.lower-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running cfork.litesec.name.capMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
