...
//
// Bootup the Masternode, look for a 10000 cfork.litesec.name.cap input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if (status != ACTIVE_MASTERNODE_STARTED) {
		...
		if (pwalletMain->GetBalance() < Params().MasternodeCollateralAmt()*COIN) {
			LogPrintf("CActiveMasternode::ManageStateInitial -- %s: Wallet balance is < 1,000 cfork.litesec.ticker\n", GetStateString());
		}
