...
*Setup instructions for a gitian build of cfork.litesec.name.cap using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the cfork.litesec.name.cap
...
to cfork.litesec.name.lower-crypto.com.
...
- [Building cfork.litesec.name.cap](#building-cfork.litesec.name.lower)
...
Clone the git repositories for cfork.litesec.name.lower and gitian and then checkout the cfork.litesec.name.lower version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/cfork.litesec.name.lower-crypto/cfork.litesec.name.lower.git
cd cfork.litesec.name.lower
...
Follow the instructions in [doc/release-process.md](release-process.md) in the cfork.litesec.name.lower repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building cfork.litesec.name.cap
----------------

To build cfork.litesec.name.cap (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the cfork.litesec.name.lower repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/cfork.litesec.name.lower/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/cfork.litesec.name.lower-crypto/cfork.litesec.name.lower
...
For example:
```bash
URL=https://github.com/crowning-/cfork.litesec.name.lower.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit cfork.litesec.name.lower=${COMMIT} --url cfork.litesec.name.lower=${URL} ../cfork.litesec.name.cap/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit cfork.litesec.name.lower=${COMMIT} --url cfork.litesec.name.lower=${URL} ../cfork.litesec.name.cap/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit cfork.litesec.name.lower=${COMMIT} --url cfork.litesec.name.lower=${URL} ../cfork.litesec.name.cap/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/cfork.litesec.name.lower-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/cfork.litesec.name.lower-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/cfork.litesec.name.lower-build.assert
```
...
[cfork.litesec.name.lower/gitian.sigs](https://github.com/cfork.litesec.name.lower-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
